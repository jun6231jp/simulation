...
float (*dT_point);
float (*dJ_point);
__global__ void grav_v(float (*pos)[3], float(*vec)[3] ,float(*sti),float(*e),float(*T),float(*J), float time, float dt);
...
__global__ void grav_v(float (*pos)[3],float(*vec)[3],float(*sti),float(*e),float(*T),float(*J), float time, float dt)
{
  ...
  for (int i = 0 ; i < NUM_POINTS; i++)
      ...
      if (dis > 2 * R * colmargin  && i != index)
	{
	  ...
	  J[index]-=0.5*M*fabs((float)(pow((double)vec[i][0]/scale,2)+pow((double)vec[i][1]/scale,2)+pow((double)vec[i][2]/scale,2))-(float)(pow((double)(vec[i][0]/scale +((pos[i][0]-xn)/dis)*gravity*ANIM),2)+pow((double)(vec[i][1]/scale + ((pos[i][1]-yn)/dis)*gravity*ANIM),2)+pow((double)(vec[i][2]/scale + ((pos[i][2]-zn)/dis)*gravity*ANIM),2)));
	  //速度更新
  ...
  if(colnum>0)
	...
	if (colindex != index) {
	  ...
	  //速度更新
	  v_buff[0]=(double)((1+repul)*M*vec[colindex][0]+(M-repul*M)*v_buff[0])/(M+M);
	  v_buff[1]=(double)((1+repul)*M*vec[colindex][1]+(M-repul*M)*v_buff[1])/(M+M);
	  v_buff[2]=(double)((1+repul)*M*vec[colindex][2]+(M-repul*M)*v_buff[2])/(M+M);
          //衝突エネルギーをstiの比で分配し熱エネルギー変換                                            
          double Energy=0.5*(1-repul*repul)*(M*(pow((double)vx/scale,2)+pow((double)vy/scale,2)+pow((double)vz/scale,2)) + M*(pow((double)vec[colindex][0]/scale,2)+pow((double)vec[colindex][1]/scale,2)+pow((double)vec[colindex][2]/scale,2)));
	  //J[index]+=Energy * pow(10.0,(double)sti[index]) / pow(10.0,(double)sti[index]) + pow(10.0,(double)sti[colindex]);
	  J[index]+=Energy * (double)sti[index] / ((double)sti[index] + (double)sti[colindex]);
	  ...
	  e[index] = e[index] * (visc+(temp/100)-(T[index]/100))/(visc); 
...
{
  for (int i = 0; i < NUM_POINTS; i++) {
    h_point[i][0] = ((double)rand()-(double)rand()) / RAND_MAX * INIT_WIDTH/30 ;
    h_point[i][1] = ((double)rand()-(double)rand()) / RAND_MAX * INIT_WIDTH/30 ;
    h_point[i][2] = ((double)rand()-(double)rand()) / RAND_MAX * INIT_WIDTH/30 ;
  }
  for (int i = 0; i < NUM_POINTS; i++) {
    v_point[i][0] = 0;
    v_point[i][1] = 0;
...
//CUDA実行関数
void launchGPUKernel(unsigned int num_particles, float (*pos)[3], float (*vec)[3] ,float(*sti),float(*e),float(*T),float(*J), float time, float dt)
{
    dim3 grid(Grid_x,Grid_y,Grid_z);
    dim3 block(Block_x,Block_y,Block_z);
    grav_v<<<grid , block>>>(pos, vec, sti, e, T, J, time, dt);
    grav_p<<<grid , block>>>(pos, vec, time, dt);
}
//アニメーション動作
void runGPUKernel(void)
{
  launchGPUKernel(NUM_POINTS, d_point, dv_point ,dst_point, de_point,dT_point,dJ_point, anim_time, anim_dt);
...
void display(void)
{
	...
	else if(T_point[k]>8000){
	  glColor3f((GLfloat)(1.0-0.25/2000*(T_point[k]-8000)),1.0f,1.0f);
	}
	else if(T_point[k]>6000){
	  glColor3f(1.0f,(GLfloat)(0.25+0.25/2000*(T_point[k]-6000)),(GLfloat)(1.0/2000*(T_point[k]-6000)));
	}
	else if(T_point[k]>2000){
	  glColor3f(1.0f,(GLfloat)(0.25+0.25/2000*(T_point[k]-2000)),0.0f);
	}
	else if(T_point[k]>0){
	  glColor3f((GLfloat)(0.25/2000*T_point[k]),0.25f,0.0f);
	}
...
void resize(int width, int height)
{
  ...
  //vision_size=vision_size*(double)window_width/((double)window_width+right_motion)*(double)window_height/((double)window_height+up_motion);
