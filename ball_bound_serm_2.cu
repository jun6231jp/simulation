...
__global__ void grav_v(float (*pos)[3],float(*vec)[3],float(*sti),float(*e),float(*T),float(*J), float time, float dt)
{
  ...
  __shared__ float coltime[NUM_POINTS][NUM_POINTS][2];
  ...
  for (int i = 0 ; i < NUM_POINTS; i++)
      ...
      if (dis > 2 * R * colmargin  && i != index)
	{
	  ...
	  J[index]-=0.5*M*fabs((float)(pow((double)vec[i][0],2)+pow((double)vec[i][1],2)+pow((double)vec[i][2],2))-(float)(pow((double)(vec[i][0] + ((pos[i][0]-xn)/dis)*gravity*ANIM),2)+pow((double)(vec[i][1] + ((pos[i][1]-yn)/dis)*gravity*ANIM),2)+pow((double)(vec[i][2] + ((pos[i][2]-zn)/dis)*gravity*ANIM),2)));
	  //速度更新
	...
	{
	  if (i != index && coltime[index][i][1] == 0){
	  //衝突域侵入からの経過の時間を記録　TBD　法線方向に直す
	  coltime[index][i][1]=(double)i+1;
	  coltime[i][index][1]=(double)index+1;
	  }
	}
    }
    __syncthreads();
  ...
  for (int i = 0 ; i < NUM_POINTS; i++)
    {
      if(coltime[index][i][1] > 0){
	coltime[index][i][0]=(float)(2*R*colmargin - dis)/(pow((double)(vx-vec[i][0]),2)+pow((double)(vy-vec[i][1]),2)+pow((double)(vz-vec[i][2]),2));
	colnum++;
      }
    }
  __syncthreads();
  ...
  if(colnum>0)
	...
	for(int j = i+1; j < NUM_POINTS; j++){
	  if(coltime[index][i][0] > coltime[index][j][0]){
	    tmp[0]=coltime[index][i][0];
	    tmp[1]=coltime[index][i][1];
	    coltime[index][i][0]=coltime[index][j][0];
	    coltime[index][i][1]=coltime[index][j][1];
	    coltime[index][j][0]=tmp[0];
	    coltime[index][j][1]=tmp[1];
	  }
	}
      }
      //衝突域侵入からの経過時間が長いものから処理
      for (int i=NUM_POINTS-1 ; i>=NUM_POINTS-colnum; i--){
	int colindex=coltime[index][i][1]-1;
	float repul=0;
	repul=e[index];
	//反発係数は小さいほうを優先     
	...
	//float Energy=0.5*(1-repul*repul)*(M*(pow((double)vx,2)+pow((double)vy,2)+pow((double)vz,2)) + M*(pow((double)vec[colindex][0],2)+pow((double)vec[colindex][1],2)+pow((double)vec[colindex][2],2)));
	//J[index]+=Energy * pow((double)M,0.667) * pow(10.0,(double)sti[index]) / (pow((double)M,0.667) * pow(10.0,(double)sti[index]) + pow((double)M,0.667) * pow(10.0,(double)sti[colindex]));
	//T[index]=(J[index]-0.5*M*(pow((double)v_buff[0],2)+pow((double)v_buff[1],2)+pow((double)v_buff[2],2)))/M/cap;	
	//粘性と反発係数の更新
	//e[index] = e[index] * exp((pow(10.0,(double)(visc-(T[index]-temp/100)))-pow(10.0,(double)sti[index]))/pow((double)M,0.5));
  ...
  //T[index]=(J[index]-0.5*M*(pow((double)vec[index][0],2)+pow((double)vec[index][1],2)+pow((double)vec[index][2],2)))/M/cap;
  //粘性と反発係数の更新 
  //e[index] = e[index] * exp((pow(10.0,(double)(visc-(T[index]-temp/100)))-pow(10.0,(double)sti[index]))/pow((double)M,0.5));
  //sti[index] = visc - (T[index] - temp / 100);
}
